...
Public Class frmOnline_Menu_Export
    ...
    Private Sub ExportCompanyInfo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExportCompanyInfo.Click
        Try
            ...
            SaveFile.FileName = Today.Year.ToString & IIf(Len(Today.Month.ToString) = 1, "0" & Today.Month.ToString, Today.Month.ToString) & IIf(Len(Today.Day.ToString) = 1, "0" & Today.Day.ToString, Today.Day) & "_Exported_" & cboCompanyName.Items(cboCompanyName.SelectedIndex).ToString & " Company_Infomation.txt"


            'OpenFile.Filter = "ASCII files (*.txt;*.)|*.txt;*.log"
            SaveFile.InitialDirectory = System.Environment.GetFolderPath(Environment.SpecialFolder.Desktop) & "\" '"D:\My Window form Applications\Restaurants\"
            ' OpenFile.ShowDialog()
    ...
    Private Sub btnExportMenu_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExportMenu.Click
        Try
            ...
            'SaveFile.Filter = "Text(*.txt)|*.txt"
            SaveFile.FileName = Today.Year.ToString & IIf(Len(Today.Month.ToString) = 1, "0" & Today.Month.ToString, Today.Month.ToString) & IIf(Len(Today.Day.ToString) = 1, "0" & Today.Day, Today.Day) & "_Exported_" & cboCompanyName.Items(cboCompanyName.SelectedIndex).ToString & " Menu.txt"
    ...
    Private Sub btnExportSpeicalMenu_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExportSpeicalMenu.Click
        Try
            ...
            SaveFile.InitialDirectory = System.Environment.GetFolderPath(Environment.SpecialFolder.Desktop) & "\" '"D:\My Window form Applications\Restaurants\"
            'SaveFile.Filter = "Text(*.txt)|*.txt"
            SaveFile.FileName = Today.Year.ToString & IIf(Len(Today.Month.ToString) = 1, "0" & Today.Month.ToString, Today.Month.ToString) & IIf(Len(Today.Day.ToString) = 1, "0" & Today.Day, Today.Day) & "_Exported_" & cboCompanyName.Items(cboCompanyName.SelectedIndex).ToString & "_SubMenu.txt"
    ...
    Private Sub frmOnline_Menu_Export_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        GetCompanies()
    End Sub
End Class